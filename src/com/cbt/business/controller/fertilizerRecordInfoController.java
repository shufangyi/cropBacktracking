...
public class fertilizerRecordInfoController {
	...
	{	
		...
		for(int i=0;i<businessCropProjectInfolist.size();i++)
		{
			...
			list = fertilizerRecordInfoService.getFertilizerRecordService(project_btCode, nowpage, rows);
			total = total+fertilizerRecordInfoService.getFertilizerRecordCountService(project_btCode, nowpage, rows);
